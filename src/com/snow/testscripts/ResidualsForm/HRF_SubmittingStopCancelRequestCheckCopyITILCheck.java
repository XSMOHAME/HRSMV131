...
public class HRF_SubmittingStopCancelRequestCheckCopyITILCheck extends ReusableLibrary {
	ReadData rd = new ReadData();

	@Test(priority = 7, dataProvider = "data", dependsOnGroups = "CreateStopCancelCheckCopyRequest")
	public void checkDissolveCorporationRequestFormITIL(Map<Object, Object> data) throws Exception {
		KnowledgeCreationPage knowledgeCreation = new KnowledgeCreationPage(driver);
		LoginPage loginPage = new LoginPage(driver);
		HRF_SubmittingStopCancelRequestCheckCopy requestForm = new HRF_SubmittingStopCancelRequestCheckCopy();
	...
	public Object[][] getData() throws IOException {
		...
		for (int i = 0; i < excelData.length; i++) {
			if (excelData[i][0].toString().contains("TestCaseFor=ITIL Check: Stop/Cancel Request: Copy Check")) {
